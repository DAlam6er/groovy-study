...
class ListRunner {
  static void main(String[] args) {
    ...
    // в старых версиях Groovy необходимо было писать в null-safe стиле list?[8]
    assert list[8] == null // list.get(8)

    // write
    list += 9                  // добавляем в конец новый элемент
    list << 11 << 13 << 7 << 7 // добавляем подряд несколько элементов
    assert list == [1, 3, 5, 6, 9, 11, 13, 7, 7]

    list -= 7                  // удаляем все вхождения числа 7
    assert list == [1, 3, 5, 6, 9, 11, 13]

    list -= [1, 3]             // удаление всех элементов, входящих в указанный список
    assert list == [5, 6, 9, 11, 13]

    list *= 3                  // повторить список 3 раза подряд
